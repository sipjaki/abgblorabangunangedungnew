...
<script>
    ...
    // Hapus event klik, jadi marker tidak bisa berpindah
        ...
        {{-- ======================================================= --}}
                    <div class="col-md-12">
                        <!--begin::Quick Example-->
                 <form action="{{ route('bapersetujuanbupatibongkar.upload', $data->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
                            <!-- begin::Body -->
                            <div class="card-body">
...
<div class="text-center">
    ...
    <h5 style="color: #0d6efd; font-weight: bold; margin-top: 5px; font-size:16px;">
        ...
        Upload Surat Persetujuan Bupati
    </h5>
    <hr class="my-4" style="border-top: 2px dashed #0d6efd; width: 60%; margin: auto;">
</div>
    {{-- Nomor Surat BUP --}}
    <div class="form-modern col-md-6 mb-3">
        <label class="form-label-modern fw-semibold">
            <i class="bi bi-file-earmark-text-fill text-primary me-1"></i> Nomor Surat Persetujuan Bupati
        </label>
        <input type="text"
               name="nomorsuratbup"
               class="form-control @error('nomorsuratbup') is-invalid @enderror"
               value="{{ old('nomorsuratbup', $data->nomorsuratbup ?? '') }}"
               placeholder="Masukkan Nomor Surat BUP">
        @error('nomorsuratbup')
    ...
    <div class="form-modern col-md-6 mb-3">
        <label class="form-label-modern fw-semibold">
            <i class="bi bi-calendar-check-fill text-success me-1"></i> Tanggal Surat Persetujuan Bupati
        </label>
        <input type="date"
               name="tanggalsuratbup"
               class="form-control @error('tanggalsuratbup') is-invalid @enderror"
               value="{{ old('tanggalsuratbup', !empty($data->tanggalsuratbup) ? \Carbon\Carbon::parse($data->tanggalsuratbup)->format('Y-m-d') : '') }}">
        @error('tanggalsuratbup')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
...
<div class="mb-3">
    ...
    @error('cadangan3')<div class="invalid-feedback">{{ $message }}</div>@enderror

    <div class="mt-3" id="previewContainerProposal" style="{{ isset($data->cadangan3) ? '' : 'display: none;' }}">
        <label class="fw-bold">Surat Persetujuan Bupati</label>
        <iframe id="iframeProposal" src="{{ isset($data->cadangan3) ? asset($data->cadangan3) : '' }}"
            style="width: 100%; height: 400px; border: 1px solid #ccc; border-radius: 6px;"></iframe>
    </div>
    <div id="msgProposal" class="mt-3"
        style="color: grey; font-style: italic; {{ isset($data->cadangan3) ? 'display:none;' : '' }}">
        Belum Upload Berkas. Silahkan Upload Surat Persetujuan Bupati
    </div>
</div>
                                ...
                                <div id="confirmModal" style="display: none; position: fixed; inset: 0; background-color: rgba(0, 0, 0, 0.5); z-index: 1000; justify-content: center; align-items: center;">
                                    <div style="background: white; padding: 24px 30px; border-radius: 12px; max-width: 400px; width: 90%; text-align: center; box-shadow: 0 10px 25px rgba(0,0,0,0.2);">
                                      <p style="font-size: 16px; font-weight: 600; margin-bottom: 20px;">
                                        Apakah Anda ingin upload Surat Persetujuan Bupati?
                                    </p>
