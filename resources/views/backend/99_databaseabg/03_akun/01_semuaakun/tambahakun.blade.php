...
<br>
        ...
        {{-- ======================================================= --}}
                    <div class="col-md-12">
                        <!--begin::Quick Example-->
                  <form action="{{ route('create.allakuncreatenew') }}" method="POST" enctype="multipart/form-data">
          @csrf
                            <!-- begin::Body -->
                            <div class="card-body">
                                <div class="row">
    ...
    <div class="col-md-6">
        ...
        <div class="mb-3">
            ...
            <input type="text" id="phone_number" name="phone_number"
                ...
                class="form-control @error('phone_number') is-invalid @enderror" placeholder="Masukkan nomor telepon" />
            @error('phone_number')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
    ...
    <div class="col-md-6">
        ...
        <div class="mb-3">
            ...
            @if (!empty($data->avatar))
                <div class="mt-2">
                    <img src="{{ asset($data->avatar) }}" alt="Avatar" style="height: 60px; border-radius: 8px;">
                </div>
            @endif
            @error('avatar')
            <div class="invalid-feedback">{{ $message }}</div>
        ...
        <div class="mb-3">
            ...
            <select name="statusadmin_id" id="statusadmin_id"
                ...
                @foreach ($statusadmins as $status)
                    <option value="{{ $status->id }}"
                        {{ old('statusadmin_id', $data->statusadmin_id ?? '') == $status->id ? 'selected' : '' }}>
                        {{ $status->status }}
                    </option>
                @endforeach
            </select>
                                ...
                                <div id="confirmModal" style="display: none; position: fixed; inset: 0; background-color: rgba(0, 0, 0, 0.5); z-index: 1000; justify-content: center; align-items: center;">
                                    <div style="background: white; padding: 24px 30px; border-radius: 12px; max-width: 400px; width: 90%; text-align: center; box-shadow: 0 10px 25px rgba(0,0,0,0.2);">
                                      <p style="font-size: 16px; font-weight: 600; margin-bottom: 20px;">
                                        Apakah Anda ingin membuat akun untuk ini  ?
                                    </p>
