...
<div class="col-md-6">
    <div class="mb-3">
        ...
        <select class="form-select @error('fungsibangunan') is-invalid @enderror" id="fungsibangunan" name="fungsibangunan">
            ...
            <option value="FUNGSI USAHA" {{ old('fungsibangunan', $penilik->fungsibangunan ?? '') == 'FUNGSI USAHA' ? 'selected' : '' }}>FUNGSI USAHA</option>
            <option value="FUNGSI HUNIAN" {{ old('fungsibangunan', $penilik->fungsibangunan ?? '') == 'FUNGSI HUNIAN' ? 'selected' : '' }}>FUNGSI HUNIAN</option>
            <option value="FUNGSI KEAGAMAAN" {{ old('fungsibangunan', $penilik->fungsibangunan ?? '') == 'FUNGSI KEAGAMAAN' ? 'selected' : '' }}>FUNGSI KEAGAMAAN</option>
            <option value="FUNGSI SOSIAL BUDAYA" {{ old('fungsibangunan', $penilik->fungsibangunan ?? '') == 'FUNGSI SOSIAL BUDAYA' ? 'selected' : '' }}>FUNGSI SOSIAL BUDAYA</option>
        </select>
    ...
    <div class="mb-3">
      ...
      <select class="form-select @error('kecamatanblora_id') is-invalid @enderror" name="kecamatanblora_id" id="kecamatanblora_id" style="min-height: 42px;">
        ...
        @foreach($kecamatanList as $item)
          <option value="{{ $item->id }}" {{ old('kecamatanblora_id', $penilik->kecamatanblora_id ?? '') == $item->id ? 'selected' : '' }}>
            {{ $item->kecamatanblora }}
          </option>
        @endforeach
      </select>
  ...
  <div class="col-md-6">
    <div class="mb-3">
      ...
      <select class="form-select @error('kelurahandesa_id') is-invalid @enderror" name="kelurahandesa_id" id="kelurahandesa_id" style="min-height: 42px;">
        ...
        @if(old('kelurahandesa_id', $penilik->kelurahandesa_id ?? false))
          <option value="{{ $penilik->kelurahandesa_id }}" selected>
            {{ optional($penilik->kelurahandesa)->desa ?? 'Kelurahan/Desa Terpilih' }}
          </option>
        @endif
      </select>
...
<script>
  ...
  $('#kecamatanblora_id').on('change', function () {
    ...
    if (kecamatanID) {
      $.ajax({
        url: '{{ route("datanewpenilik.create") }}',
        type: 'GET',
                                ...
                                <div id="confirmModal" style="display: none; position: fixed; inset: 0; background-color: rgba(0, 0, 0, 0.5); z-index: 1000; justify-content: center; align-items: center;">
                                    <div style="background: white; padding: 24px 30px; border-radius: 12px; max-width: 400px; width: 90%; text-align: center; box-shadow: 0 10px 25px rgba(0,0,0,0.2);">
                                      <p style="font-size: 16px; font-weight: 600; margin-bottom: 20px;">
                                        Apakah Anda ingin memperbarui berkas penilik ini ?
                                    </p>
