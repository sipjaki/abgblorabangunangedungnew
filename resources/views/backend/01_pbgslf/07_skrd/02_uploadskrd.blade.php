...
<br>
        ...
        {{-- ======================================================= --}}
                    <div class="col-md-12">
                        <!--begin::Quick Example-->
                  <form action="{{ route('create.bepbgslfskrdcreatenew', $data->id) }}" method="POST" enctype="multipart/form-data">
          @csrf
...
<div class="form-modern col-md-6">
    <div class="mb-3">
        ...
        <input
            type="text"
            class="form-control"
            value="{{ $data->tanggalpermohonan ? \Carbon\Carbon::parse($data->tanggalpermohonan)->translatedFormat('d F Y') : '-' }}"
            readonly
            style="background-color: #e9ecef; cursor: not-allowed;"
        >
    </div>
...
<script>
    ...
    rupiahInput.addEventListener('input', function() {
        ...
        // Simpan angka raw tanpa titik ke hidden input
        rupiahRaw.value = getRawNumber(value);

        // Format ulang input dengan titik ribuan
        this.value = formatRupiah(rupiahRaw.value);
    });
</script>
...
<div class="form-modern col-md-6">
  <div class="mb-3">
    ...
    @if(!empty($data->berkasskrd) && file_exists(public_path($data->berkasskrd)))
      <div class="mb-3">
        <label class="fw-bold">Berkas SKRD Sebelumnya:</label>
        @php
          $fileUrl = asset($data->berkasskrd);
          $ext = strtolower(pathinfo($data->berkasskrd, PATHINFO_EXTENSION));
        @endphp
        ...
        @else
          <a href="{{ $fileUrl }}" target="_blank" class="btn btn-primary">
            Download Berkas SKRD Sebelumnya
          </a>
        @endif
      </div>
    @endif
                                ...
                                <div id="confirmModal" style="display: none; position: fixed; inset: 0; background-color: rgba(0, 0, 0, 0.5); z-index: 1000; justify-content: center; align-items: center;">
                                    <div style="background: white; padding: 24px 30px; border-radius: 12px; max-width: 400px; width: 90%; text-align: center; box-shadow: 0 10px 25px rgba(0,0,0,0.2);">
                                      <p style="font-size: 16px; font-weight: 600; margin-bottom: 20px;">
                                        Apakah Anda ingin upload berkas ini ?
                                    </p>
