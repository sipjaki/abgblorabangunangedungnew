...
<div class="text-center">
   ...
   <h5 class="text-primary fw-bold mt-2" style="font-size: 16px;">
    ...
    Pemilihan Petugas TPT/TPA
</h5>
</h5>
    <hr class="my-4" style="border-top: 2px dashed #0d6efd; width: 60%; margin: auto;">
</div>

<form id="formPemilik" action="{{ route('bepbgtpatptcreatenew2') }}" method="POST">
    @csrf
...
<div class="row g-4 mt-2">
    ...
    <select name="timpenilai" class="form-select @error('timpenilai') is-invalid @enderror">
        ...
        <option value="TPA" {{ old('timpenilai') == 'TPA' ? 'selected' : '' }}>TPA</option>
        <option value="TPT" {{ old('timpenilai') == 'TPT' ? 'selected' : '' }}>TPT</option>
    </select>
   ...
   <div class="form-modern col-md-6">
    ...
    <select name="nosk" class="form-select @error('nosk') is-invalid @enderror">
        ...
        @foreach (collect($pengawasList)->unique('nosk') as $item)
            <option value="{{ $item->nosk }}" {{ old('nosk') == $item->nosk ? 'selected' : '' }}>
                {{ $item->nosk }}
            </option>
        @endforeach
    </select>
...
<div id="confirmModal" style="display: none; position: fixed; inset: 0; background-color: rgba(0, 0, 0, 0.5); z-index: 1000; justify-content: center; align-items: center;">
    <div style="background: white; padding: 24px 30px; border-radius: 12px; max-width: 400px; width: 90%; text-align: center; box-shadow: 0 10px 25px rgba(0,0,0,0.2);">
        <p style="font-size: 16px; font-weight: 600; margin-bottom: 20px;">Apakah Anda ingin menyimpan data pemilihan TPA/TPT ini?</p>
