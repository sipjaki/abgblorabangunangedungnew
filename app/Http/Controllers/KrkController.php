...
public function permohonankrkusaha(Request $request)
{
    ...
    public function permohonankrkusahacreate(Request $request)
    {
        ...
        krkusaha::create([
            ...
            'alamatpemohon' => $validatedData['alamatpemohon'] ?? null,
...
public function destroykrkusahasurat($id)
{
    ...
    if ($data) {
        ...
        return redirect()->route('bekrkindex')->with('delete', 'Data berhasil dihapus.');
    } else {
        // Redirect dengan flash message error
        return redirect()->route('bekrkindex')->with('error', 'Data tidak ditemukan.');
    }
}
...
public function permohonankrkhunianfinalman($id)
{
    // Ambil data utama krkusaha berdasarkan ID
    $datausaha = krkhunian::where('id', $id)->first();
...
public function permohonankrkkeagamaanfinalman($id)
{
    // Ambil data utama krkusaha berdasarkan ID
    $datausaha = krkkeagamaan::where('id', $id)->first();
...
public function bekrksosbudperbaikannewadmin(Request $request, $id)
{
    $bantuan = krksosbud::findOrFail($id);
    ...
    'tanggalpermohonan'  => 'nullable|date',
    'notelepon'          => 'nullable|string|max:255',
    'luastanah'          => 'nullable|integer|min:0',
...
public function permohonankrkhuniancreate(Request $request)
    ...
    public function permohonankrksosbudcreate(Request $request)
    {
        ...
        $validatedData = $request->validate([
            ...
            'kabupaten' => 'required|string|max:255',
            'kecamatanblora_id' => 'required|string|max:255',
            'kelurahandesa_id' => 'nullable|string|max:255',
            'lokasibangunan' => 'required|string',
            'alamatpemohon' => 'required|string',
...
$datagsbkabblora = rencanagsbblora::withoutGlobalScopes()
    ...
    public function permohonankrkusahafinalmanual($id)
{
    // Ambil data utama krkusaha berdasarkan ID
    $datausaha = krkusaha::where('id', $id)->first();
    ...
    // Return ke view
    return view('backend.06_krk.01_pengesahanusaha.08_ttdmanual', [
        'title' => 'Berkas Final Permohonan KRK Fungsi Usaha',
        'data' => $datausaha,       // Data utama krkusaha
        'subdata' => $datasurat,    // Data sub krkusahasurat
        'datagsb' => $datagsb,      // Data dropdown/GSB
...
public function perpengesahankrkagama(Request $request, $id)
{
...
public function dokuploadkrkagamanew(Request $request, $id)
{
    // Validasi input
    $validated = $request->validate([
        'suratuploadmanual' => 'nullable|file|mimes:pdf|max:10048',
...
public function validasikrkmenara(Request $request, $id)
{
  ...
  public function validasikrkmenara1(Request $request, $id)
    {
        $data = krkmenara::findOrFail($id);
...
public function validasikrkmenara3(Request $request, $id)
    ...
    public function validasikrkmenara4(Request $request, $id)
    {
        $data = krkmenara::findOrFail($id);
...
public function dokuploadkrkmenaranew(Request $request, $id)
{
    // Validasi input
    $validated = $request->validate([
        'suratuploadmanual' => 'nullable|file|mimes:pdf|max:10048',
...
public function bekrkmenarapemohon(Request $request)
{
    $user = Auth::user();
    $search = $request->input('search');
    $perPage = $request->input('perPage', 15);
    ...
    // Query dasar: hanya data milik user yang login
    $query = krkmenara::where('user_id', $user->id);
    ...
    // Ambil data sub dari relasi krkusahasurat berdasarkan krksosbud_id
    $subdata = krkusahasurat::whereIn('krkmenara_id', $krksosbudIds)->get();

    return view('backend.00_pemohon.06_krk.05_menara.01_berkasmenara', [
        'title' => 'Permohonan KRK Menara Telekomunikasi',
        'data' => $berkasusaha,
        'subdata' => $subdata,
        'user' => $user,
