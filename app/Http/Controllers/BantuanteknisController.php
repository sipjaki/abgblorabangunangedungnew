...
{
    // ✅ VALIDASI (support semua file)
    $validated = $request->validate([
        'bantuanteknis_id' => 'required|string',
        'kegiatan' => 'required|string',
        'tanggalkegiatan' => 'required|date',

        'foto1' => 'required|file|mimes:jpg,jpeg,png,pdf,doc,docx,xls,xlsx,ppt,pptx|max:7048',
        'foto2' => 'nullable|file|mimes:jpg,jpeg,png,pdf,doc,docx,xls,xlsx,ppt,pptx|max:7048',
        'foto3' => 'nullable|file|mimes:jpg,jpeg,png,pdf,doc,docx,xls,xlsx,ppt,pptx|max:7048',
        'foto4' => 'nullable|file|mimes:jpg,jpeg,png,pdf,doc,docx,xls,xlsx,ppt,pptx|max:7048',
        'foto5' => 'nullable|file|mimes:jpg,jpeg,png,pdf,doc,docx,xls,xlsx,ppt,pptx|max:7048',
        'foto6' => 'nullable|file|mimes:jpg,jpeg,png,pdf,doc,docx,xls,xlsx,ppt,pptx|max:7048',
    ]);

    $data = new ceklapanganbantek();

    $data->bantuanteknis_id = $validated['bantuanteknis_id'];
    $data->kegiatan = $validated['kegiatan'];
    $data->tanggalkegiatan = $validated['tanggalkegiatan'];

    // 🔥 LOOP upload (biar gak ngulang kode)
    for ($i = 1; $i <= 6; $i++) {
        ...
        if ($request->hasFile($field)) {
            ...
            // folder dinamis (01,02,03...)
            $folder = '04_bantuanteknis/0' . $i . '_ceklapangan';
    ...
    return redirect()->route('bebantuanasistensilap.show', [
        'id' => $validated['bantuanteknis_id']
    ]);
}
...
public function bebantekkonsultannewjasa(Request $request)
{
    ...
    $data->namalengkap = $validated['namalengkap'] ?? null ;
    $data->no_telepon = $validated['no_telepon'] ?? null ;
...
public function bebantekpembongkaranall(Request $request)
{
    $user    = Auth::user();
    $search  = $request->input('search');
    $perPage = $request->input('perPage', 10);
 ...
 public function bebantekkerusakandokumen($namagedung, $id)
{
    ...
    // Ambil data berdasarkan ID (bukan first)
    $data = bantekanalisainduk::where('id', $id)->firstOrFail();

    // Validasi slug hanya jika namagedung tidak kosong
